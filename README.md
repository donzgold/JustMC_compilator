...
"переменная ${a}"
"фигня ${variable::add([1,2])}"
"Фигня ${любое значение}"
```
#### Переменные
...
//Можно использовать переменную вместо селектора
...
```ts
@item("stone_sword") //установить отображаемый предмет
...
// (так же, если сначала указана переменная, она заменит случайную переменную для возврата)
